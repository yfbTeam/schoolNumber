...
<head>
        ...
        <tr>
            ...
            <td>
                <span class="enable_wrap">{{if Status == 0}}
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'0','PersonDocument')" class="enable">启用</a>
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'1','PersonDocument')">归档</a>
                    {{else}}
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'0','PersonDocument')">启用</a>
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'1','PersonDocument')" class="disable">归档</a>
                    {{/if}}
                </span>
            </td>
    ...
    <script id="trTran" type="text/x-jquery-tmpl">
        <tr>
            ...
            <td>
                <span class="enable_wrap">{{if Status == 0}}
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'0','TrainingFiles')" class="enable">启用</a>
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'1','TrainingFiles')">归档</a>
                    {{else}}
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'0','TrainingFiles')">启用</a>
                    <a href="javascript:void(0);;" onclick="ChangeStatus(${ID},'1','TrainingFiles')" class="disable">归档</a>
                    {{/if}}
                </span>
            </td>
            <td>${TrainResult}</td>
             <td>
                <a style="cursor:pointer;" onclick="javascript:OpenIFrameWindow('查看考试档案', '/PersonalSpace/TrainExam.aspx?trainID=${ID}', '600px', '410px')">考试</a>
                <a style="cursor:pointer;" onclick="javascript:OpenIFrameWindow('查看课程档案', '/PersonalSpace/TrainCouse.aspx?trainID=${ID}', '600px', '410px')">课程</a>
            </td>
        </tr>
        ...
        <header class="repository_header_wrap manage_header">
            <div class="width repository_header clearfix">
                ...
                <nav class="navbar menu_mid fl">
                    <ul id="CourceMenu">
                        <li class="active" id="0"><a href="#" onclick="ChangeType(0)">学员档案</a></li>
                    ...
                    <div class="wrap">
                        <div class="newcourse_select clearfix">
                            ...
                            <div class="stytem_select_right fr" style="height: auto;">
                                ...
                                <a onclick="UpdateDoc()" id="icon-plus" style="cursor:pointer">更新学员档案                            
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="stytem_select clearfix">
                            <div class="stytem_select_left fl">
                                <a href="javascript:;" class="on">当前档案信息</a>
                                <a href="javascript:;">归档档案信息</a>
                            </div>
                        </div>
                    ...
                    <div class="wrap none">
                        <div class="newcourse_select clearfix">
                            <div class="fl" style="margin-top: 5px;">
                                <label style="float: left; line-height: 38px; display: block;">图表类型：</label>
                                <select class="select" id="select_type3">
                                    <option value="1" selected>3D</option>
                                    <option value="2">2D</option>
                                </select>
                            </div>
                        ...
                        <div id="divPieChart3" align="center" style="width: 1000px; height: 600px; margin: 0 auto;"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    function initChart1() {
        var xml1 = "<graph  caption='当前档案信息汇总' formatNumberScale='0' numberSuffix='元'>"
                + "<categories >"
    ...
    function initChart2() {
        var xml2 = "<graph  caption='归档档案信息汇总' formatNumberScale='0' numberSuffix='元'>"
                + "<categories >"

                + "<category name='2016/07/14' />"
                + "<category name='2012/03/02' />"
                + "</categories>"
                + "<dataset seriesName='王小明' color='1D8BD1' anchorBorderColor='1D8BD1' anchorBgColor='1D8BD1'>"
                        + "<set value='6500' />"
                        + "<set value='' />"
                + "</dataset>"
                + "<dataset seriesName='方晓' color='1D8sD1' anchorBorderColor='1D8BD1' anchorBgColor='1D8BD1'>"
                        + "<set value='7500' />"
                        + "<set value='' />"
                + "</dataset>"
                + "<dataset seriesName='林彤' color='1hgsD1' anchorBorderColor='1D8BD1' anchorBgColor='1D8BD1'>"
                        + "<set value='1000' />"
                        + "<set value='' />"
                + "</dataset>"
                + "<dataset seriesName='罗秋香' color='58hBD1' anchorBorderColor='1D8BD1' anchorBgColor='1D8BD1'>"
                        + "<set value='' />"
                        + "<set value='5000' />"
                ...
                + "</dataset>"
                 ...
                 + "</graph>";
        var myChart = new FusionCharts("/FusionCharts/Swf/MSColumnLine3D.swf", "myChartId_04", '1000', "600");
    ...
    function initChart3() {
        var xml3 = "<graph  formatNumberScale='0' numberSuffix='元' caption='培训档案汇总'>"
                + "<categories >"
                ...
                + "<set value='5000' />"
                + "</dataset></graph>";
        var myChart = new FusionCharts("/FusionCharts/Swf/MSColumnLine3D.swf", "myChartId_05", '1000', "600");
        myChart.setDataXML(xml3);
        myChart.render("divPieChart3");
    }
    initChart3();
    $('#select_type3').change(function () {
        ...
        if (val == 1) {
            ...
            var myChart = echarts.init(document.getElementById('divPieChart3'));
            option3 = {
                title: {
                    text: '参加工作时间汇总',
                    x: 'center'
...
<script type="text/javascript">
    ...
    function MyDocDetail(IDCard) {
        OpenIFrameWindow('个人档案详情', '/PersonalSpace/MyDocDetail.aspx?IDCard=' + IDCard, '800px', '90%');
    };
    function MyDocUpdate(IDCard) {
        OpenIFrameWindow('个人档案修改', '/PersonalSpace/MyDocEdit.aspx?IDCard=' + IDCard, '800px', '90%');
    };
