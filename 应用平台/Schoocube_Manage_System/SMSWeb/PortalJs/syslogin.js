$(function () {
    ...
    $("#GoBBS").on("click", function () {
        if ($.cookie('LoginCookie_Cube') == null || $.cookie('LoginCookie_Cube') == "null" || $.cookie('LoginCookie_Cube') == "") {
            $(this).attr("href", "http://192.168.1.101:9010/index.aspx");
        } else {
            var info = $.parseJSON($.cookie('LoginCookie_Cube'));
            var param = "user=" + encrypt(info.LoginName) + "&pwd=" + encrypt($.cookie("PwdCookie_Cube"));
            $(this).attr("href", "http://192.168.1.101:8072/InterfaceUser.aspx?" + param);
