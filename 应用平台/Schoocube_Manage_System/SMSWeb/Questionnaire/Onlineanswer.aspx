<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Onlineanswer.aspx.cs" Inherits="SMSWeb.Questionnaire.Onlineanswer" %>
    ...
    function obtain() {
        ...
        for (var i = 0; i < contrastanswer.length; i++) {
            $.ajax({
                url: "/Common.ashx",//random" + Math.random(),//方法所在页面和方法名
                type: "post",
                data: { PageName: "/Exam/ExamHandler.ashx", "action": "addExam_ExamAnswer", "ExamID": exid, "QuestionID": contrastanswer[i].ID, "ExampaperID": id, "Type": 4, "Answer": wAnswer[i], "Score": 0 },
                success: function (json) { }
            });
        }
        //addSysNotice(zdrname + examname, zdrname + "待阅试卷", 1, hIDCard, hnamee, "", "");
        window.location.href = "MyOption.aspx?id=" + id + "&name=" + hIDCard;
    }
    var thisid = "";
