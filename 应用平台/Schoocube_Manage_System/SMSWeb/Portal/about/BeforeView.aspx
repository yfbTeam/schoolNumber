<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BeforeView.aspx.cs" Inherits="SMSWeb.Portal.about.BeforeView" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <asp:HiddenField ID="HSpecialId" runat="server"/>
        <input  type="hidden" id="HadvName" />
       <div class="top">
    ...
    <script type="text/javascript">
        ...
        function initData() {
                    ...
                    success: function (json) {
                        if (json.result.errMsg == "success") {
                            ...
                            if (item != null) {
                                ...
                                $("#noticeImg").html("<img src='" + item.ImageUrl + "'/>");
            ...
            } else if ($("#HSpecialId").val().length > 0) {
                $.ajax({
                    type: "Post",
                    url: "/Common.ashx",
                    async: false,
                    ...
                    success: function (json) {
                        if (json.result.errMsg == "success") {
                            var item = json.result.retData;
                            if (item != null && item.length > 0) {
                                $("#HadvName").val(item[0].Description);
                                $("#noticeTitle").html(item[0].Description);
                                $("#Contents").html(item[0].CreativeHTML);
                            }
                        }
        ...
        function leftTree() {
            if ($("#HSpecialId").val().length > 0) {
                var html = "<a href=\"#\" data-url=\"/Portal/about/BeforeView.aspx?id=" + $("#HSpecialId").val() + "\" class=\"on\" >" + $("#HadvName").val() + "</a>";
                $("#div_leftTree").html(html);
                $("#aTypeMenu").html($("#HadvName").val());
                $("#aTypeMenu").attr("href", "/Portal/about/BeforeView.aspx?id=" + $("#HSpecialId").val());
                $("#hTitle").html($("#HadvName").val());
                var enName = "";
                if ($("#HadvName").val().indexOf("联系") > -1) enName = "Contact US";
                else enName = " Web Introduction";
                $("#szTitle").html(enName);
                return;
            }
            $.ajax({
