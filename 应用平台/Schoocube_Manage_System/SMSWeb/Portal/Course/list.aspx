...
<body>
    <form id="form1" runat="server">
        ...
        <input type="hidden" id="HisCharge"  runat="server"/>
        <input type="hidden" id="HCourseType" runat="server" />
        <div class="top">
        ...
        <div class="main width clearfix  mb20" style="margin-top:20px;">
            ...
            <div class="content">
                ...
                <div class="search_wrap">
                    ...
                    <input type="button" name="query" value="查询" id="btnQuery" />
                    <input type="button" value="自定义"  id="customQuery" />
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#customQuery").on('click', function () {
                layer.open({
                    ...
                    area: ['380px', '130px'], //宽高
                    content: "<input type=\"checkbox\" name=\"chkCustomIsCharge\" value=\"0\" onclick=\"clickCharge(this)\" />免费<input type=\"checkbox\" name=\"chkCustomIsCharge\" value=\"1\" onclick=\"clickCharge(this)\"  />收费<input type=\"checkbox\" name=\"chkCustomType\" value=\"1\" onclick=\"clickCourseType(this)\"  />线下课程<input type=\"checkbox\" name=\"chkCustomType\" value=\"2\" onclick=\"clickCourseType(this)\"  />线上课程<input type=\"button\"  value=\"查询\" id=\"CustomCourseQuery\" />"
                });
                if ($("input[name='chkCustomIsCharge']")) {
                    var isp = $("#HisCharge").val();
                    if (isp != "") {
                        if (isp == "-1111")
                            $('input[name=chkCustomIsCharge]').prop('checked', true);
                        else
                            $('input[name=chkCustomIsCharge][value=' + isp + ']').prop('checked', true);
                    }
                }
                if ($("input[name='chkCustomType']")) {
                    var cct = $("#HCourseType").val();
                    if (cct != "") {
                        if (cct == "-1111")
                            $('input[name=chkCustomType]').prop('checked', true);
                        else
                            $('input[name=chkCustomType][value=' + cct + ']').prop('checked', true);
                    }
                }
                $("#CustomCourseQuery").click(function () {
            ...
            $("#CourseFavorite").on("click", function () {
                ...
                if ($.cookie('LoginCookie_Cube') == null || $.cookie('LoginCookie_Cube') == "null" || $.cookie('LoginCookie_Cube') == "") {
                    OpenIFrameWindow('用户登录', '/Portal/login.aspx?url=/Portal/Course/list.aspx', '700px', '500px');
                } else {
                    layer.open({
                        type: 1,
                        skin: 'layui-layer-rim', //加上边框
                        area: ['380px', '130px'], //宽高
                        content: "<div class=\"preview_Favor\" style=\"padding: 20px;\"><input  id=\"FavorName\" type=\"text\" placeholder=\"请输入收藏夹名称\"/><input type=\"button\"  value=\"确定\" onclick=\"AddFavorite()\"  /></div>"
                    });
                }
        ...
        function clickCharge(eve)
        {
            var i = 0;
            var selVal = "";
            $("input[name='chkCustomIsCharge']").each(function (index, ele) {
                if ($(ele).is(":checked")) {
                    i++;
                    selVal = $(ele).val();
                }
            ...
            {
                $("#HisCharge").val(selVal);
            }
        }

        function clickCourseType(eve)
        {
            var i = 0;
            var selVal = "";
            $("input[name='chkCustomType']").each(function (index, ele) {
                if ($(ele).is(":checked")) {
                    i++;
                    selVal = $(ele).val();
                }
            ...
            if (i == 2) {
                $("#HCourseType").val("-1111");
            } else {
                $("#HCourseType").val(selVal);
            }
        }
        ...
        function getData(startIndex, pageSize) {
            ...
            if ($.cookie('LoginCookie_Cube') == null || $.cookie('LoginCookie_Cube') == "null" || $.cookie('LoginCookie_Cube') == "") {
                $.ajax({
                    ...
                    success: function (json) {
                        if (json.result.errMsg == "success") {
                            ...
                            makePageBar(getData, document.getElementById("pageBar"), json.result.retData.PageIndex, 0, 10, 0);
                            $("#tb_data").html("<li>暂无课程！</li>");
                        }
                    },
                ...
                if ($("#HRoleType").val() == "学生") {
                    var ct = $("#HCourseType").val();
                    if (ct == "-1111") ct = "";
                    var ic = $("#HisCharge").val();
                    if (ic == "-1111") ic = "";
                    $.ajax({
                        url: "/Common.ashx",
                        type: "post",
        ...
        function AddFavorite() {
            var urlpath = "";
            if (window.location.href.indexOf("coursename")>-1) {
                urlpath = window.location.href.substring(0, window.location.href.indexOf("coursename")-1);
            } else {
                urlpath = window.location.href;
            }
            var url = urlpath + "&coursename=" + $("#CourseName").val() + "&ischarge=" + $("#HisCharge").val() + "&coursetype=" + $("#HCourseType").val();
            var title = $("#FavorName").val();
            $.ajax({
                url: "/Common.ashx",
                type: "post",
                async: false,
        ...
        function hrefShow(cid) {
            if ($.cookie('LoginCookie_Cube') != null && $.cookie('LoginCookie_Cube') != "null" && $.cookie('LoginCookie_Cube') != "") {
                if ($("#HRoleType").val() == "学生") {
                    ...
                    return "/CourseManage/CourseDetail.aspx?itemid=" + cid ;
                }
            } else {
